        ...
        with open(DATA_FILE, "r", encoding="utf-8", newline="") as f:
            ...
            for row in reader:
                try:
                    ...
                    days = int(row[6])
                    status = LeaveStatus[row[7]]
                    who_confirmed = row[8] if row[8] else None

                    # Pomijamy __init__ (który robi load_users) - ustawiamy pola ręcznie
                    req = object.__new__(LeaveRequest)
                    req.employee_id = employee_id
                    req.first_name = first_name
                    req.last_name = last_name
                    req.start_date = start_date
                    req.end_date = end_date
                    req.amount_days = days
                    req.status = status
...
def save_leave_requests(requests: Dict[int, LeaveRequest]):
    ...
    try:
        with open(DATA_FILE, "w", encoding="utf-8", newline="") as f:
            writer = csv.writer(f)
            writer.writerow([
                "request_id", "employee_id", "first_name", "last_name",
                "start_date", "end_date", "days", "status", "who_confirmed"
            ])
...
def create_leave_request(requests: Dict[int, LeaveRequest], employee, start_date: date,
                         end_date: date, days: int) -> LeaveRequest:
